...
public class BMAdapter extends RecyclerView.Adapter<BMAdapter.MyViewHolder> {

    AQuery aq;
    //ImageLoader imageLoader = AppController.getInstance().getImageLoader();
    private Context mContext;
    private List<BanglaMusicVideo> fullVideoClassList;

    public BMAdapter(Context context, List<BanglaMusicVideo> fullVdoHomeList){
        this.mContext = context;
        this.fullVideoClassList = fullVdoHomeList;
    }

    @Override
    public BMAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_hd,parent,false);
        //aq = new AQuery(view);
        return new BMAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(BMAdapter.MyViewHolder holder, int position) {

        BanglaMusicVideo primaryClass = fullVideoClassList.get(position);
