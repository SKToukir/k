...
public class HDVideoAdapter extends RecyclerView.Adapter<HDVideoAdapter.MyViewHolder> {

    AQuery aq;
    //ImageLoader imageLoader = AppController.getInstance().getImageLoader();
    private Context mContext;
    private List<HDVideo> fullVideoClassList;

    public HDVideoAdapter(Context context, List<HDVideo> fullVdoHomeList){
        this.mContext = context;
        this.fullVideoClassList = fullVdoHomeList;
    }

    @Override
    public HDVideoAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_hd,parent,false);
        //aq = new AQuery(view);
        return new HDVideoAdapter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(HDVideoAdapter.MyViewHolder holder, int position) {

        HDVideo primaryClass = fullVideoClassList.get(position);
